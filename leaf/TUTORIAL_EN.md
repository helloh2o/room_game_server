...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://server/leaf/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
A game server implemented with Leaf may include many modules (e.g. [LeafServer](https://server/leafserver)) which all share below traits:

* Each module runs inside a separate goroutine
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://server/leaf/tree/master/chanrpc))
...
[LeafServer](https://server/leafserver) is a game server developped with Leaf. Let's start with it.

Download the source code of LeafServer：

```
git clone https://server/leafserver
```
...
More references are at [leaf/go](https://server/leaf/blob/master/go)。
...
More references are at [leaf/timer](https://server/leaf/blob/master/timer)。
...
More references are at [leaf/log](https://server/leaf/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://server/leaf/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://server/leaf/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://server/leaf/wiki](https://server/leaf/wiki)
