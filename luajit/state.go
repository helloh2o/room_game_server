...
/*
#cgo !darwin LDFLAGS: -lluajit -L "jit"
#cgo linux LDFLAGS: -lm -ldl
#cgo darwin LDFLAGS: -lluajit_darwin -L "jit"

#include <jit/lua.h>
#include <jit/lauxlib.h>
#include <jit/luajit.h>
#include <jit/lualib.h>
#include <stddef.h>
#include <stdlib.h>
...
//export docallback
func docallback(fp *C.char, sp unsafe.Pointer) int {
	fn := C.GoString(fp)
	state := State{((*C.lua_State)(sp))}
	addrNum, _ := strconv.ParseUint(fn, 10, 64)
	//fmt.Println("callback address:", addrNum)

	//fnn := *(*func(*State) int)(unsafe.Pointer((uintptr(addrNum))))

	//return fnn(&state)
	callback, ok := callbackMap.Load(addrNum)
	if ok {
		callback1 := callback.(Gofunction)
		return callback1(&state)
	}
...
func (s *State) Pushstring(str string) {
	...
	defer C.free(unsafe.Pointer(cs))
	// C.lua_pushstring(s.l, cs)
	C.lua_pushlstring(s.l, cs, C.ulong(len(str)))
}
...
func (s *State) Register(fn Gofunction, name string) {
	var addrNum = *(*uint64)(unsafe.Pointer(&fn))
	callbackMap.Store(addrNum, fn)
	//fmt.Println("register address:%d", addrNum)
	s.Pushstring(strconv.FormatUint(addrNum, 10))
	C.pushclosure(s.l, C.int(0))
	s.Setglobal(name)
}
...
// convert for int64
func (s *State) Tointeger64(index int) int64 {
	return int64(C.lua_tointeger(s.l, C.int(index)))
}
...
// return table  {"AA",{name="ay","aaa",ar ={"a","b","c"},content={134,2,99,22,3}}},"xx",true
func (s *State) ToTable(index int) interface{} {
	var ret interface{}
	s.Pushvalue(index)
	s.Pushnil()
	var isMap bool
	for s.Next(-2) > 0 {
		...
		//log.Printf("key=%s, value=%s", s.Tostring(-1), s.Tostring(-2))
		//log.Printf("value type %v", s.Type(-2))
		if isMap {
			key := s.Tostring(-1)
			switch {
			case s.Istable(-2):
				ret.(map[string]interface{})[key] = s.ToTable(-2)
			case s.Isstring(-2):
				ret.(map[string]interface{})[key] = s.Tostring(-2)
			case s.Isnumber(-2):
				var value interface{}
				v1 := s.Tointeger64(-2)
				v2 := s.Tonumber(-2)
			...
			case s.Isboolean(-2):
				ret.(map[string]interface{})[key] = s.Toboolean(-2)
			default:
				ret.(map[string]interface{})[key] = s.Tostring(-2)
			}
		} else {
			switch {
			case s.Istable(-2):
				ret = append(ret.([]interface{}), s.ToTable(-2))
			case s.Isstring(-2):
				ret = append(ret.([]interface{}), s.Tostring(-2))
			case s.Isnumber(-2):
				var value interface{}
				v1 := s.Tointeger64(-2)
				v2 := s.Tonumber(-2)
				...
				ret = append(ret.([]interface{}), value)
			case s.Isboolean(-2):
				ret = append(ret.([]interface{}), s.Toboolean(-2))
			default:
				log.Printf("unsupport type %v", s.Type(-2))
				ret = append(ret.([]interface{}), s.Tostring(-2))
			}
		}
		s.Pop(2)
