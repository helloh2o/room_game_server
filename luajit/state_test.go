...
func TestLoadFile(t *testing.T) {
	...
	if err := s.Loadfile("../game/test/test2.lua"); err != nil {
		t.Fatalf("%s -- %s", err.Error(), s.Tostring(-1))
	}
	if err := s.Pcall(0, Multret, 0); err != nil {
		t.Fatalf("%s -- %s", err.Error(), s.Tostring(-1))
	}
	if err := s.Loadfile("../game/test/test1.lua"); err != nil {
		t.Fatalf("%s -- %s", err.Error(), s.Tostring(-1))
	}
	if err := s.Pcall(0, Multret, 0); err != nil {
		t.Fatalf("%s -- %s", err.Error(), s.Tostring(-1))
	...
	s.Getglobal("onmessage")
	s.Pushinteger(100273)
	s.Pushstring("message from client")
...
`
	...
	s.Register(func(s *State) int {
		m := s.Tostring(-1)
		r := s.Tointeger(-2)
		log.Printf("go onmessage %d, %s", r, m)
		return 0
	}, "send_msg")
	s.Loadstring(code)
	s.Pcall(0, 0, 0)
	...
	for {
		...
		s.Getglobal("onmessage")
		s.Pushstring(a.ToJson())
		err := s.Pcall(1, 0, 0) // 这样不会挂
		if err != nil {
			log.Println(err)
		}
		//s.Call(1,0) // 这样会挂
...
`
	tk := time.Tick(time.Millisecond * 200)
	for i := 0; i < 100; i++ {
		<-tk
		go func(code string) {
			s := Newstate()
			s.Openlibs()
			err := s.Loadstring(code)
			if err != nil {
				log.Println(err)
				log.Println(s.Tostring(-1))
			}
			err = s.Pcall(0, 0, 0)
			if err != nil {
				log.Println(err)
